...
class MacroController:
    ...
    def loop(self):
        ...
        if self.loop_count % self.reset_navmap_loop_count == 0 and self.loop_count != 0:
            ...
            self.terrain_analyzer.calculate_navigation_map()
            numbers = list(range(len(self.terrain_analyzer.platforms.keys())))
            random.shuffle(numbers)
            if self.navmap_reset_type == 1:
                for idx, (key, platform) in enumerate(self.terrain_analyzer.platforms.items()):
        ...
        if rune_coords := self.screen_processor.find_rune_marker():
            ...
            if rune_solve_time_offset >= self.player_manager.rune_cooldown or rune_solve_time_offset <= 30:
                ...
                if rune_platform_hash:
                    self.logger.debug(f"rune on platform {rune_platform_hash}")
        ...
        self.logger.debug(
            f"next solution destination: {next_platform_solution.to_hash} method: {next_platform_solution.method}"
        )
        ...
        if self.player_manager.x >= next_platform_solution.lower_bound[0] and self.player_manager.x <= next_platform_solution.upper_bound[0]:
            ...
            in_solution_movement_goal = (
                lookahead_ub
                if abs(
                    self.player_manager.x - next_platform_solution.lower_bound[0]
                )
                < abs(
        ...
        elif self.player_manager.x < next_platform_solution.lower_bound[0]:
            # We are left of solution bounds.
        ...
        elif random.randint(0, self.walk_probability) == 1:
